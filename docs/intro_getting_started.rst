...
前言
````````
现在你已经阅读了安装指南并安装了Ansible。是时候通过一些命令开始深入了解Ansible了。
我们最先展示的并非那强大的集配置，部署，自动化于一身的playbook，相关内容将在另一章节中讲述。
...
Ansible 1.3及之后的版本默认会在本地的 OpenSSH可用时会尝试用其进行远程通讯。这会启用ControlPersist（一个性能特性），Kerberos和 在~/.ssh/config中的配置选项如 Jump Host setup。
然而，当你使用Linux企业版6作为主控机（红帽企业版及其衍生版如CentOS），其OpenSSH版本可能过于老旧无法支持ControlPersist。
